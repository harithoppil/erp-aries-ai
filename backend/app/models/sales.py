...
class DeliveryNote(Base, TimestampMixin, AuditMixin):
    ...
    total_qty: Mapped[float] = mapped_column(default=0.0)
    items: Mapped[list["DeliveryNoteItem"]] = relationship("DeliveryNoteItem", back_populates="delivery_note", cascade="all, delete-orphan")
...
class SalesInvoice(Base, TimestampMixin, AuditMixin):
    ...
    enquiry_id: Mapped[uuid.UUID | None] = mapped_column(GUID(), ForeignKey("enquiries.id"), nullable=True)
    ...
    items: Mapped[list["SalesInvoiceItem"]] = relationship("SalesInvoiceItem", back_populates="sales_invoice", cascade="all, delete-orphan")
