...
async def list_pages(current_user=Depends(get_current_user)):
    return wiki_service.list_pages()


@router.get("/pages/{page_path:path}", response_model=WikiPageRead)
async def get_page(page_path: str, current_user=Depends(get_current_user)):
...
async def create_page(data: WikiPageCreate, current_user=Depends(get_current_user)):
...
async def update_page(page_path: str, data: WikiPageUpdate, current_user=Depends(get_current_user)):
...
async def delete_page(page_path: str, commit_message: str = "Delete page", current_user=Depends(get_current_user)):
    wiki_service.delete_page(page_path, commit_message)


@router.get("/search", response_model=list[WikiSearchResult])
async def search_wiki(q: str, limit: int = 10, current_user=Depends(get_current_user)):
    return wiki_service.search(q, limit=limit)


@router.get("/index")
async def get_index(current_user=Depends(get_current_user)):
