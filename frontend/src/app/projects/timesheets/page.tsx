...
export default function TimesheetsPage() {
  ...
  async function loadItems() {
    ...
    try {
      const res = await apiFetch(`/projects/timesheets?page=${page}&page_size=${pageSize}${search ? `&search=${search}` : ""}`);
      setItems(res.data || []);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <a href="/projects/timesheets/new" className="flex items-center gap-2 px-4 py-2 bg-primary hover:bg-primary/90 text-primary-foreground text-sm font-medium rounded-lg transition-colors">
            ...
            {loading ? (
              ...
              items.map((item: any) => (
                <tr key={item.id} className="hover:bg-muted/50 transition-colors">
                  <td className="px-4 py-3 text-sm font-medium text-foreground">{item.employee_name}</td><td className="px-4 py-3 text-sm text-muted-foreground">{item.project_name}</td><td className="px-4 py-3 text-sm text-muted-foreground">{formatDate(item.start_date)}</td><td className="px-4 py-3 text-sm text-muted-foreground">{item.total_hours}</td>
