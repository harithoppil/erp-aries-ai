  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-navy">Quotations</h1>
          <p className="text-sm text-gray-500">Manage your Quotations. Total: {total}</p>
        </div>
        <a href="/sales/quotations/new" className="flex items-center gap-2 px-4 py-2 bg-gold hover:bg-[#B08D2F] text-white text-sm font-medium rounded-lg transition-colors">
        ...
        <table className="w-full">
          ...
          <tbody className="divide-y divide-gray-100">
            {loading ? (
              ...
              <tr><td colSpan={6} className="px-4 py-8 text-center text-sm text-gray-400">No Quotations found</td></tr>
            ) : (
              items.map((item: any) => (
                <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-4 py-3 text-sm font-medium text-navy">{item.series}</td><td className="px-4 py-3 text-sm text-gray-600">-</td><td className="px-4 py-3 text-sm text-gray-600">{formatDate(item.date)}</td><td className="px-4 py-3"><span className={`px-2 py-0.5 text-xs rounded-full ${getStatusColor(item.status)}`}>{item.status}</span></td><td className="px-4 py-3 text-sm font-medium text-navy">{formatMoney(item.grand_total)}</td>
