...
export default function NotebookEditorPage() {
  ...
  const handleAIAssist = async () => {
    ...
    setAiResponse("");
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000/api/v1"}/ai/chat/claude`, {
        method: "POST",
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              ...
              {aiResponse && (
                <div className="space-y-2">
                  <div className="rounded-lg border bg-slate-50 p-3 text-sm text-slate-700 max-h-64 overflow-y-auto">
                    {aiResponse}
                  </div>
