...
export default function DiveOperationsPage() {
  ...
  async function loadItems() {
    ...
    try {
      const res = await apiFetch(`/marine/dive-operations?page=${page}&page_size=${pageSize}${search ? `&search=${search}` : ""}`);
      setItems(res.data || []);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-foreground">Dive Operations</h1>
          <p className="text-sm text-muted-foreground">Total: {total}</p>
        </div>
        <a href="/marine/dive-operations/new" className="flex items-center gap-2 px-4 py-2 bg-primary hover:bg-primary/90 text-primary-foreground text-sm font-medium rounded-lg transition-colors">
            ...
            {loading ? (
              ...
              items.map((item: any) => (
                <tr key={item.id} className="hover:bg-muted/50 transition-colors">
                  <td className="px-4 py-3 text-sm font-medium text-foreground">{item.operation_id}</td><td className="px-4 py-3 text-sm text-muted-foreground">{formatDate(item.operation_date)}</td><td className="px-4 py-3 text-sm text-muted-foreground">{item.vessel}</td><td className="px-4 py-3 text-sm text-muted-foreground">{item.dive_location}</td>
