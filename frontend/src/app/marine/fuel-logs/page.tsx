...
export default function FuelLogsPage() {
  ...
  async function loadItems() {
    ...
    try {
      const res = await apiFetch(`/marine/fuel-logs?page=${page}&page_size=${pageSize}${search ? `&search=${search}` : ""}`);
      setItems(res.data || []);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-foreground">Fuel Logs</h1>
          <p className="text-sm text-muted-foreground">Total: {total}</p>
        </div>
        <a href="/marine/fuel-logs/new" className="flex items-center gap-2 px-4 py-2 bg-primary hover:bg-primary/90 text-primary-foreground text-sm font-medium rounded-lg transition-colors">
            ...
            {loading ? (
              ...
              items.map((item: any) => (
                <tr key={item.id} className="hover:bg-muted/50 transition-colors">
                  <td className="px-4 py-3 text-sm text-muted-foreground">{formatDate(item.date)}</td><td className="px-4 py-3 text-sm font-medium text-foreground">{item.vessel}</td><td className="px-4 py-3 text-sm text-muted-foreground">{item.fuel_type}</td><td className="px-4 py-3 text-sm text-muted-foreground">{item.qty}</td>
