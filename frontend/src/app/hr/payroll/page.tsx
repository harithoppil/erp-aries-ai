...
export default function PayrollPage() {
  ...
  async function loadItems() {
    ...
    try {
      const res = await apiFetch(`/hr/payroll?page=${page}&page_size=${pageSize}${search ? `&search=${search}` : ""}`);
      setItems(res.data || []);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <a href="/hr/payroll/new" className="flex items-center gap-2 px-4 py-2 bg-primary hover:bg-primary/90 text-primary-foreground text-sm font-medium rounded-lg transition-colors">
      ...
      <div className="bg-card rounded-xl shadow-sm border border-border">
        ...
        <table className="w-full">
          <thead className="bg-muted">
            <tr>
              <th className="px-4 py-3 text-left text-xs font-medium text-muted-foreground uppercase">Employee</th><th className="px-4 py-3 text-left text-xs font-medium text-muted-foreground uppercase">Period</th><th className="px-4 py-3 text-left text-xs font-medium text-muted-foreground uppercase">Gross Pay</th><th className="px-4 py-3 text-left text-xs font-medium text-muted-foreground uppercase">Net Pay</th>
            ...
            {loading ? (
              ...
              items.map((item: any) => (
                <tr key={item.id} className="hover:bg-muted/50 transition-colors">
                  <td className="px-4 py-3 text-sm font-medium text-foreground">{item.employee_name}</td><td className="px-4 py-3 text-sm text-muted-foreground">{item.salary_slip_based_on}</td><td className="px-4 py-3 text-sm font-medium text-foreground">{formatMoney(item.gross_pay)}</td><td className="px-4 py-3 text-sm font-medium text-foreground">{formatMoney(item.net_pay)}</td>
