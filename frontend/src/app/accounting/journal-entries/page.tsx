...
export default function JournalEntriesPage() {
  ...
  async function loadItems() {
    ...
    try {
      const res = await apiFetch(`/accounting/journal-entries?page=${page}&page_size=${pageSize}${search ? `&search=${search}` : ""}`);
      setItems(res.data || []);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-foreground">Journal Entries</h1>
          <p className="text-sm text-muted-foreground">Total: {total}</p>
        </div>
        <a href="/accounting/journal-entries/new" className="flex items-center gap-2 px-4 py-2 bg-primary hover:bg-primary/90 text-primary-foreground text-sm font-medium rounded-lg transition-colors">
            ...
            {loading ? (
              ...
              items.map((item: any) => (
                <tr key={item.id} className="hover:bg-muted/50 transition-colors">
                  <td className="px-4 py-3 text-sm font-medium text-foreground">{item.series}</td><td className="px-4 py-3 text-sm text-muted-foreground">{formatDate(item.posting_date)}</td><td className="px-4 py-3"><span className={`px-2 py-0.5 text-xs rounded-full ${getStatusColor(item.status)}`}>{item.status}</span></td><td className="px-4 py-3 text-sm font-medium text-foreground">{formatMoney(item.total_debit)}</td>
