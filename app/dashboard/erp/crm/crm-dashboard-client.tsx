...
type CRMDashboardClientProps = {
  ...
  trendData: LeadConversionTrendPoint[];
};
...
function DesktopChartCard({ trendData }: { trendData: LeadConversionTrendPoint[] }) {
  const [refreshing, setRefreshing] = useState(false);
  const [data, setData] = useState(trendData);

  const handleRefresh = async () => {
    setRefreshing(true);
    const fresh = await getLeadConversionTrends();
    setData(fresh);
    setRefreshing(false);
  };
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-xs text-[#64748b] mt-0.5">
            Monthly leads and opportunities over the past 12 months
          </p>
        </div>
        ...
        <ResponsiveContainer width="100%" height={280}>
          <LineChart data={data} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
            ...
            <Line
              ...
              stroke={CHART_COLORS.opportunities}
              strokeWidth={2}
              strokeDasharray="5 5"
              dot={{ r: 3, fill: CHART_COLORS.opportunities }}
              activeDot={{ r: 5 }}
              name="Opportunities"
            />
          </LineChart>
...
function MobileChartCard({ trendData }: { trendData: LeadConversionTrendPoint[] }) {
  ...
  const kpiCards = (
    <>
      ...
      <KpiCard
        ...
        value={dashboardData.openOpportunities.toLocaleString()}
        subtext="Not closed or lost"
        accentBg="bg-emerald-50 text-emerald-600"
      />
      <KpiCard
        icon={FileText}
        label="Active Contracts"
        value={dashboardData.activeContracts.toLocaleString()}
    ...
    <>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
        ...
        <div className="divide-y divide-gray-50">
          <MasterLink icon={Users} label="Leads" href="/dashboard/erp/crm/leads" />
          <MasterLink icon={Target} label="Opportunities" href="/dashboard/erp/crm/opportunities" />
          <MasterLink icon={FileText} label="Contracts" href="/dashboard/erp/crm/contracts" />
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
        ...
        <div className="divide-y divide-gray-50">
          ...
          <MasterLink icon={Bell} label="Notification" href="#" />
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
        ...
        <div className="divide-y divide-gray-50">
          <MasterLink icon={Settings} label="CRM Settings" href="/dashboard/settings" />
        </div>
      </div>
    </>
  );
