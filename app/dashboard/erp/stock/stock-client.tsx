...
import { listItems, listWarehouses, listStockEntries, createStockEntry, type ClientSafeItem, type ClientSafeWarehouse, type ClientSafeStockEntry } from "@/app/dashboard/erp/stock/actions";
import { usePageContext } from "@/hooks/usePageContext";
import {
  Package, CheckCircle, AlertTriangle, ShieldAlert,
  Search, Warehouse, Plus, Download, Wand2, Sparkles,
} from "lucide-react";
    ...
    registerActions(
      [
        defineAction({
          ...
          description: "Open and fill the create stock entry form with the provided details. Opens dialog and fills all fields in one shot.",
          parameters: {
            type: "object",
            required: ["entry_type", "quantity"],
            properties: {
              entry_type: { type: "string", description: "Type of stock entry (required)", enum: ["receipt", "delivery", "transfer"] },
              item_id: { type: "string", description: "Item ID" },
              quantity: { type: "number", description: "Quantity (required)" },
              source_warehouse: { type: "string", description: "Source warehouse ID" },
              target_warehouse: { type: "string", description: "Target warehouse ID" },
              reference: { type: "string", description: "Reference number or note" },
            },
          },
        }),
        defineAction({
          name: "set_stock_search",
          description: "Filter the stock list by search term",
          parameters: {
            type: "object",
            required: ["term"],
            properties: {
        ...
        <div className="space-y-4 pb-4">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div className="flex gap-2">
              <Button variant="outline" className="gap-2 rounded-xl" onClick={() => exportToCSV(filtered.map(i => ({ item_name: i.item_name, item_code: i.item_code, item_group: i.item_group, unit: i.unit, quantity: i.quantity, unit_cost: i.unit_cost })), 'stock')}>
