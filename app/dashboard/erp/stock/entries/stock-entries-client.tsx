...
import { ArrowRightLeft, Search, Plus, List, SlidersHorizontal } from "lucide-react";
...
const fmt = (v: number) =>
  v.toLocaleString("en-AE", { style: "currency", currency: "AED" });
...
export default function StockEntriesClient({
  ...
  const filtered = useMemo(() => {
    ...
    return entries.filter((e) => {
      ...
      const matchesId =
        !filters.id || (e.name || "").toLowerCase().includes(filters.id.toLowerCase());
      const matchesType =
        !filters.stockEntryType ||
        (e.stock_entry_type || "")
          .toLowerCase()
          .includes(filters.stockEntryType.toLowerCase());
      const matchesSource =
        !filters.sourceWarehouse ||
        (e.from_warehouse || "")
          .toLowerCase()
          .includes(filters.sourceWarehouse.toLowerCase());
      const matchesTarget =
        !filters.targetWarehouse ||
        (e.to_warehouse || "")
          .toLowerCase()
          .includes(filters.targetWarehouse.toLowerCase());
      return matchesSearch && matchesId && matchesType && matchesSource && matchesTarget;
    });
  ...
  const handleCreate = async (ev: React.FormEvent) => {
    ...
    const result = await createStockEntry({
      ...
      items: [{ item_code: form.item_code, qty: parseFloat(form.qty) }],
    });
    if (result.success) {
      ...
      const res = await listStockEntries();
  ...
  const clearFilters = () => {
    setFilters({ id: "", stockEntryType: "", sourceWarehouse: "", targetWarehouse: "" });
    setSearch("");
  };

  const hasActiveFilters =
    filters.id || filters.stockEntryType || filters.sourceWarehouse || filters.targetWarehouse;
        ...
        <div className="space-y-4 pb-4">
          ...
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbPage>Stock Entry</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <h2 className="text-2xl font-bold text-[#0f172a]">Stock Entries</h2>
            ...
            <div className="flex gap-2">
              ...
              <ExportButton
                data={filtered.map((e) => ({
                  name: e.name,
                  type: e.stock_entry_type,
                  date: dt(e.posting_date),
                  from: e.from_warehouse,
                  to: e.to_warehouse,
                  value: e.value_difference,
                }))}
          ...
          <div className="relative">
            ...
            <Input
              placeholder="Search by name, type, warehouse..."
              value={search}
              ...
              <div className={`grid ${isMobile ? "grid-cols-1" : "grid-cols-4"} gap-3`}>
                ...
                <div>
                  ...
                  <Input
                    ...
                    value={filters.stockEntryType}
                    onChange={(e) =>
                      setFilters({ ...filters, stockEntryType: e.target.value })
                    }
                    className="h-9 text-sm"
                  />
                </div>
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setFilters({ ...filters, sourceWarehouse: e.target.value })
                    }
                    className="h-9 text-sm"
                  />
                </div>
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setFilters({ ...filters, targetWarehouse: e.target.value })
                    }
                    className="h-9 text-sm"
                  />
                </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            {filtered.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 text-[#94a3b8]">
                <ArrowRightLeft size={48} className="mb-4 opacity-40" />
                <p className="text-lg font-medium">No stock entries found</p>
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {filtered.map((e) => {
                      const tc =
                        TYPE_COLORS[e.stock_entry_type] ||
                        "bg-gray-100 text-gray-700 border-gray-200";
                      return (
                        <tr
                          key={e.name}
                          onClick={() =>
                            router.push(`/dashboard/erp/stock/entries/${e.name}`)
                          }
                        ...
                        >
                          ...
                          <td className="px-4 py-3">
                            <span
                              className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium border ${tc}`}
                            >
                              {e.stock_entry_type}
                            </span>
                          </td>
                          ...
                          <td className="px-4 py-3 text-xs text-[#64748b]">
                            {e.from_warehouse || "—"} &rarr;{" "}
                            {e.to_warehouse || "—"}
                          </td>
                          <td
                            className={`px-4 py-3 text-right font-semibold ${
                              e.value_difference >= 0
                                ? "text-green-700"
                                : "text-red-700"
                            }`}
                          >
                            {fmt(Math.abs(e.value_difference))}
                          </td>
                        </tr>
                      );
                    })}
          ...
          <form onSubmit={handleCreate} className="space-y-3">
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setForm({ ...form, stock_entry_type: e.target.value })
                }
              >
