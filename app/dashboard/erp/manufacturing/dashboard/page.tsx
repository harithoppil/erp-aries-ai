...
import DashboardClient from '@/app/dashboard/erp/manufacturing/dashboard/DashboardClient';

export const dynamic = 'force-dynamic';

export default async function ManufacturingDashboardPage() {
  try {
    const [bOM, workOrders, jobCards, productionPlans] = await Promise.all([
      prisma.bOM.count(),
    ...
    const chartData = await getMonthlyData('workOrder');

    const kpis = { bOM, workOrders, jobCards, productionPlans };

    return <DashboardClient kpis={kpis} chartData={chartData} />;
  } catch (error) {
    return (
      <DashboardClient
        kpis={{ bOM: 0, workOrders: 0, jobCards: 0, productionPlans: 0 }}
        chartData={[]}
      />
    );
  }
