...
import { listProjects, listTasks, createProject, createTask, type ClientSafeProject, type ClientSafeTask } from "@/app/dashboard/erp/projects/actions";
...
interface ProjectsClientProps {
  initialProjects: ClientSafeProject[];
  initialTasks: ClientSafeTask[];
}

export default function ProjectsClient({ initialProjects, initialTasks }: ProjectsClientProps) {
  const router = useRouter();
  const [projects, setProjects] = useState<ClientSafeProject[]>(initialProjects);
  ...
  const [tasks, setTasks] = useState<ClientSafeTask[]>(initialTasks);
  ...
  useEffect(() => {
    registerActions(
      [
        defineAction({
          name: "create_project",
          description: "Open and fill the create project form with the provided details. Opens dialog and fills all fields in one shot.",
          parameters: {
            type: "object",
            required: ["project_name", "project_type", "customer_name"],
            properties: {
              ...
              estimated_cost: { type: "number", description: "Estimated cost in AED" },
              day_rate: { type: "number", description: "Day rate in AED" },
              expected_start: { type: "string", description: "Expected start date (YYYY-MM-DD)" },
              expected_end: { type: "string", description: "Expected end date (YYYY-MM-DD)" },
            },
          },
        }),
        defineAction({
          name: "create_task",
          description: "Open and fill the create task form with the provided details. Opens task dialog and fills all fields in one shot.",
          parameters: {
            type: "object",
            required: ["subject"],
            properties: {
              ...
              assigned_to: { type: "string", description: "Assigned to (personnel name)" },
        ...
        defineAction({
          name: "set_project_search",
          description: "Filter the project list by search term",
          parameters: {
            type: "object",
            required: ["term"],
            properties: {
