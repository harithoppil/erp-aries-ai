...
import { ArrowLeft, User, Calendar, FileText, DollarSign, FileInput } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Table, TableHeader, TableBody, TableHead, TableRow, TableCell } from "@/components/ui/table";
import { submitPurchaseInvoice, cancelPurchaseInvoice } from "../actions";
import { toast } from "sonner";

interface Item { name: string; item_code: string | null; item_name: string; qty: number; uom: string; rate: number; amount: number; expense_account: string | null; cost_center: string | null }
interface PIRecord { name: string; supplier: string; supplier_name: string | null; posting_date: string; due_date: string | null; status: string; grand_total: number; net_total: number; total_taxes_and_charges: number; outstanding_amount: number; paid_amount: number; currency: string; is_return: boolean; is_paid: boolean; on_hold: boolean; docstatus: number; company: string | null; project: string | null; bill_no: string | null; remarks: string | null; credit_to: string; tax_id: string | null; items: Item[] }
...
export default function PurchaseInvoiceDetailClient({ record }: { record: PIRecord }) {
  const router = useRouter(); const [loading, setLoading] = useState(false);
  const cfg = STATUS[record.status] || STATUS.Draft;
  return (
    <div className="max-w-5xl mx-auto p-6 space-y-6">
      <div className="flex items-center gap-4">
        <Button variant="ghost" size="icon" onClick={() => router.push("/dashboard/erp/buying/invoices")}><ArrowLeft size={20} /></Button>
        <div className="flex-1"><h1 className="text-2xl font-bold text-[#0f172a]">{record.name}</h1><p className="text-sm text-[#64748b] mt-1">Purchase Invoice</p></div>
        <span className={`inline-flex items-center rounded-full px-3 py-1 text-xs font-medium border ${cfg.badge}`}>{cfg.label}</span>
        {record.docstatus === 0 && <Button onClick={async () => { setLoading(true); const r = await submitPurchaseInvoice(record.name); if (r.success) { toast.success("Submitted"); router.push("/dashboard/erp/buying/invoices"); } else toast.error(r.error); setLoading(false); }} disabled={loading} className="bg-[#1e3a5f] hover:bg-[#152a45]">Submit</Button>}
        {record.docstatus === 1 && <Button variant="destructive" onClick={async () => { setLoading(true); const r = await cancelPurchaseInvoice(record.name); if (r.success) { toast.success("Cancelled"); router.push("/dashboard/erp/buying/invoices"); } else toast.error(r.error); setLoading(false); }} disabled={loading}>Cancel</Button>}
      </div>
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 space-y-3"><div className="flex items-center gap-2 text-[#64748b]"><User size={16} /><span className="text-xs font-medium uppercase">Supplier</span></div><p className="font-semibold text-[#0f172a]">{record.supplier_name || record.supplier}</p>{record.tax_id && <p className="text-xs text-[#94a3b8]">Tax ID: {record.tax_id}</p>}</div>
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 space-y-3"><div className="flex items-center gap-2 text-[#64748b]"><FileInput size={16} /><span className="text-xs font-medium uppercase">Bill</span></div><p className="font-medium">{record.bill_no || "—"}</p><p className="text-sm">Posting: <span className="font-medium">{dt(record.posting_date)}</span></p><p className="text-sm">Due: <span className="font-medium">{dt(record.due_date)}</span></p></div>
