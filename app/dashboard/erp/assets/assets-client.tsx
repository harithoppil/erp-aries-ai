...
import { listAssets, createAsset, type ClientSafeAsset } from "@/app/dashboard/erp/assets/actions";
...
export default function AssetsClient({ initialAssets }: { initialAssets: ClientSafeAsset[] }) {
  const router = useRouter();
  const [assets, setAssets] = useState<ClientSafeAsset[]>(initialAssets);
  ...
  useEffect(() => {
    registerActions(
      [
        defineAction({
          name: "create_asset",
          description: "Open and fill the create asset form with the provided details. Opens dialog and fills all fields in one shot.",
          parameters: {
            type: "object",
            required: ["asset_name", "asset_code"],
            properties: {
              asset_name: { type: "string", description: "Asset name (required)" },
              asset_code: { type: "string", description: "Unique asset code (required)" },
              asset_category: { type: "string", description: "Asset category", enum: ["navigation", "communication", "safety", "machinery", "tools", "other"] },
              location: { type: "string", description: "Asset location" },
              purchase_cost: { type: "number", description: "Purchase cost in AED" },
              calibration_date: { type: "string", description: "Calibration date (YYYY-MM-DD)" },
              next_calibration_date: { type: "string", description: "Next calibration due date (YYYY-MM-DD)" },
            },
          },
        }),
        defineAction({
          name: "set_asset_search",
          description: "Filter the asset list by search term",
          parameters: {
            type: "object",
            required: ["term"],
            properties: {
