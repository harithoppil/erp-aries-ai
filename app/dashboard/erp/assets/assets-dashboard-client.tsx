...
function DesktopChartCard({ trendData }: { trendData: AssetTrendPoint[] }) {
  const [refreshing, setRefreshing] = useState(false);
  const [data, setData] = useState(trendData);

  const handleRefresh = async () => {
    setRefreshing(true);
    const fresh = await getAssetTrends();
    setData(fresh);
    setRefreshing(false);
  };
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-xs text-[#64748b] mt-0.5">
            Monthly asset acquisitions and value over the past 12 months
          </p>
        </div>
          ...
          <LineChart data={data} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
            ...
            <Tooltip
              ...
              formatter={(value: any, name: any) => {
                if (name === 'totalValue') return [formatAED(value), 'Total Value'];
                return [value, 'Assets Added'];
              }}
...
function MobileChartCard({ trendData }: { trendData: AssetTrendPoint[] }) {
  ...
  const kpiCards = (
    <>
      <KpiCard
        icon={Wrench}
        label="Total Assets"
        value={dashboardData.assetCount.toLocaleString()}
      ...
      <KpiCard
        ...
        value={formatAED(dashboardData.totalValue)}
        subtext="Value after depreciation (AED)"
        accentBg="bg-emerald-50 text-emerald-600"
      />
      <KpiCard
        icon={MapPin}
        label="Locations"
        value={dashboardData.locationCount.toLocaleString()}
        subtext="Distinct asset locations"
    ...
    <>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
        <h4 className="text-sm font-semibold text-[#0f172a] mb-3 flex items-center gap-2">
          <Package size={14} className="text-[#1e3a5f]" />
          Assets
        </h4>
        <div className="divide-y divide-gray-50">
          <MasterLink icon={Wrench} label="Asset" href="/dashboard/erp/assets/list" />
          <MasterLink icon={MapPin} label="Location" href="#" />
          <MasterLink icon={Layers} label="Asset Category" href="#" />
          <MasterLink icon={Truck} label="Asset Movement" href="#" />
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
        <h4 className="text-sm font-semibold text-[#0f172a] mb-3 flex items-center gap-2">
          <WrenchIcon size={14} className="text-[#1e3a5f]" />
          Maintenance
        </h4>
        <div className="divide-y divide-gray-50">
          <MasterLink icon={Building2} label="Asset Maintenance Team" href="#" />
          <MasterLink icon={ClipboardCheck} label="Asset Maintenance" href="#" />
          <MasterLink icon={FileText} label="Asset Maintenance Log" href="#" />
          <MasterLink icon={DollarSign} label="Asset Value Adjustment" href="#" />
          <MasterLink icon={Wrench} label="Asset Repair" href="#" />
          <MasterLink icon={Package} label="Asset Capitalization" href="#" />
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
        ...
        <div className="divide-y divide-gray-50">
          <MasterLink icon={FileText} label="Asset Depreciation Ledger" href="#" />
          <MasterLink icon={BarChart3} label="Asset Depreciations and Balances" href="#" />
          <MasterLink icon={ClipboardCheck} label="Asset Maintenance" href="#" />
          <MasterLink icon={TrendingUp} label="Asset Activity" href="#" />
        </div>
      </div>
    </>
  );
