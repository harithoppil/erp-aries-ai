...
> {
  try {
    ...
    if (params?.search) {
      where.OR = [
        { project_name: { contains: params.search, mode: 'insensitive' } },
        { project_code: { contains: params.search, mode: 'insensitive' } },
        { customer_name: { contains: params.search, mode: 'insensitive' } },
        { vessel_name: { contains: params.search, mode: 'insensitive' } },
      ];
    }
    if (params?.status) {
      where.status = params.status;
    }
    const projects = await prisma.projects.findMany({ where, orderBy: { created_at: 'desc' } });
...
> {
  try {
    ...
    const tasks = await prisma.tasks.findMany({ where, orderBy: { start_date: 'desc' } });
  ...
  try {
    ...
    const project = await prisma.projects.create({
      data: {
        ...
        expected_end: validated.expected_end || null,
